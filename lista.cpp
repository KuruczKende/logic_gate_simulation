 ...
 * @param i the Idx of the element to retrieve
 *
 * @return the element at Idx i
 */
template<typename T>
T lista<T>::operator[](size_t i) const{
    if (this->length() > i)
        return *(this->getin(i));
    throw "over Idxed";
}
 ...
 */
template<typename T>
const T* lista<T>::getin(size_t i) const {
    if (i == 0)
        return &elem;
    return next->getin(i - 1);
}
...
//size_t
template<>
size_t lista<size_t>::length() const {
    if(next==nullptr)
        return 0;
    return next->length()+1;
}
...
//port_t*
template<>
size_t lista<port<module_t*>>::length() const{
    if (next == nullptr)
        return 0;
    return next->length() + 1;
}
...
template<>
port<module_t*> lista<port<module_t*>>::operator[](size_t i) const{
    if (length() > i)
        return *getin(i);
    throw "over Idxed";
}
...
//prot_module_t*
template<>
size_t lista<prot_module_t*>::length() const {
    if(next==nullptr)
        return 0;
    return next->length()+1;
}
...
template<>
prot_module_t* lista<prot_module_t*>::operator[](size_t i) const{
    if (length() > i)
        return *getin(i);
    throw "over Idxed";
}
...
template<>
size_t lista<char*>::length() const{
    if (next == nullptr)
        return 0;
    return next->length() + 1;
}
...
template<>
char* lista<char*>::operator[](size_t i) const{
    if (length() > i)
        return *getin(i);
    throw "over Idxed";
}
