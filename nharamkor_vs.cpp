...
uint8_t modulesteszt(const char* str, lista<prot_module_t*>& modules) {
    ...
    while (k < strlen(str)) {
        ...
        size_t in = ref->get_be_num();
        size_t out = ref->get_ki_num();
        v = k = v + 1;
 ...
 */
bool instruct_handler(char* s, module_t*& m_main, uint8_t& mods, size_t& number, lista<module_t*>& wait_to_do_modules) {
    ...
    for (size_t i = 0; i < 26; i++)
        if(inputs[i]!='?')
            if (inputs[i] != m_main->get_be_ertek(i)) {
                m_main->set_be(i, inputs[i]);
                m_main->add_to_list(wait_to_do_modules,i);
            }
    return true;
}
/**
...
bool print_module_error(uint8_t err) {
    ...
    case 2:  std::cout << "nem _ val kezdodo nev\n"; return false;
    case 3:  std::cout << "rossz karakter a nevben\n"; return false;
    case 4:  std::cout << "mar foglalt nev\n"; return false;
    case 5:  std::cout << "masodik kivezetes comp_module-ban\n"; return false;
    case 6:  std::cout << "nem jo bekotes comp_module-ban\n"; return false;
    case 7:  std::cout << "nemletezo module comp_module-ban\n"; return false;
    case 8:  std::cout << "rossz inputszam comp_module-ban\n"; return false;
    case 9:  std::cout << "rossz outputszam comp_module-ban\n"; return false;
    case 10: std::cout << "nem jo bekotes text_module-ban\n"; return false;
    case 11: std::cout << "nem jo forma text_module-ban\n"; return false;
    case 12: std::cout << "rossz karakterek a text_module-ban\n"; return false;
...
void print(module_t*& m_main, bool kezd = true, bool lezar = true) {
    ...
    size_t in = m_main->get_be_num();
    size_t out = m_main->get_ki_num();
    if (kezd) {
        std::cout << (char)218;
    ...
    std::cout << (char)179;
    for (size_t j = 0; j < in; j++)std::cout << (char)m_main->get_be_ertek(j);
    std::cout << (char)179;
    for (size_t j = 0; j < out; j++)std::cout << (char)m_main->get_ki_ertek(j);
    std::cout << (char)179 << '\n';
    if (lezar) {
        std::cout << (char)192;
        for (size_t i = 0; i < in; i++)std::cout << (char)196;
...
void input_handler(std::istream& in, lista<module_t*>& wait_to_do_modules, uint8_t& mods, lista<prot_module_t*>& modulok, module_t*& m_main, lista<char*>& insts);
/**
 ...
 */
void input_handler_module(char* s, lista<prot_module_t*>& modulok, module_t*& m_main) {
 ...
 */
void input_handler_read(char* s, lista<module_t*>& wait_to_do_wires, uint8_t& mods, lista<prot_module_t*>& modulok, module_t*& m_main, lista<char*>& insts) {
    ...
    while (!inf.eof()) {
        input_handler(inf, wait_to_do_wires, mods, modulok, m_main, insts);
    }
    inf.close();
}
/**
 ...
 */
bool input_handler_do(char* s, lista<module_t*>& wait_to_do_modules, uint8_t& mods, module_t*& m_main) {
    size_t number;
    bool addmain = false;
    if(!instruct_handler(s, m_main, mods, number, wait_to_do_modules))return false;
...
void input_handler(std::istream& in, lista<module_t*>& wait_to_do_modules, uint8_t& mods, lista<prot_module_t*>& modulok, module_t*& m_main, lista<char*>& insts) {
    ...
    insts.add(s);
    if (s[0] == '_') {//új module
        input_handler_module(s, modulok, m_main);
    }
    else if (s[0] == '<') {//fájlból olvasás
        input_handler_read(s, wait_to_do_modules, mods, modulok, m_main, insts);
    }
    else if (s[0] == '>') {//fájlba írás
        input_handler_write(s, insts);
    }
    else {//végrehajtás
        if(!input_handler_do(s, wait_to_do_modules, mods, m_main))std::cout<<"rossz vegrehajtas utasitas\n";
    }
}
...
int main()
{
    ...
    while (!((mode&0b10000000) == 0b10000000)) {
        input_handler(std::cin, wait_to_do_modules, mode, modulok, m_main, insts);
    }
